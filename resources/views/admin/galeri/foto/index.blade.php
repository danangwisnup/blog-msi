            ...
            <div class="row">
                <div class="col-12">
                    <div class="text-end">
                        ...
                        <a href="<?= url('admin/galeri/foto/create') ?>" class="btn btn-success">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <table class="table border table-striped table-bordered align-middle text-center"
                                ...
                                <tbody>
                                    @foreach ($ct_foto as $item)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td>{{ $item->nama }}</td>
                                            <td><img src="{{ url('img') }}/{{ $item->foto }}"
                                                    class="img-fluid img-thumbnail" style="max-width: 200px" alt="Foto">
                                            <td class="text-center">
                                                <a href="{{ route('foto.show', $item->id) }}" class="btn btn-primary">
    ...
    <script>
        function confirmDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch('<?= url('admin/galeri/foto') ?>/' + id, {
                        method: 'DELETE',
                        headers: {
        ...
        function confirmDeleteAll() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch('<?= url('admin/galeri/foto/destroy-all') ?>', {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '<?= csrf_token() ?>'
                        }
